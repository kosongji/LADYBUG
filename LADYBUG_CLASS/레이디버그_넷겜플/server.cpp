...
void gameValueInit();
float bug_x_move(Bug bug, Player player);
...
void ITEM6_DIRECTION(Item &item_6, int i)
{
...
void ITEM6_MOVE(Item &item_6, int i)
{
	if (item_6_direction[i] == 0)
	{
		item_6.y -= 13;
...
void ITEM8(Player &player, Item &item_8, int i)
{
...
void ITEM10(Player &player, Item &item_10, int i)
{
...
//아이템드롭 + 플레이어 충돌
int IsItemCollisionCheck(Item &item, Player &player)
{
	//for (int i = 0; i < MONSTER; ++i)
	{
		//아이템 top에서 충돌
...
float bug2_x_move(Bug bug, Item item)
{
	float bug_lean = ((item.y - bug.y) / (item.x - bug.x));
	bug.x_move = bug.y_move / bug_lean;
	return bug.x_move;
}
...
void gameTimerFunc()
{
	if (START && (!Gameover))
	{
		...
		for (int i = 0; i < 10; ++i)
		{
			...
			if (item_10_Flag[i] == 1)
			{
				bug[bug_num].x_move = bug2_x_move(bug[bug_num], item_10[i]);// 버그의 x이동값 정하기
			}
		}
